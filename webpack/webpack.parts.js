    ...
    rules: [
      {
        ...
        use: [
          ...
          //     resources: [
          //       path.resolve(__dirname, '../app/assets/styles/utils/variables.scss'),
          //       path.resolve(__dirname, '../app/assets/styles/utils/functions.scss')
          //     ]
          //   }
          // }
        ]
...
exports.extractCSS = ({include, exclude, use} = {}) => {
  ...
  const plugin = new ExtractTextPlugin({filename: '[name].[contenthash:8].css'})
  return {
    module: {
      rules: [
        {
...
exports.page = ({
  ...
  // console.log('StagingPage' + isStaging) console.log('prodPage' + isProd)
  ...
  return {
    entry,
    output,
    plugins: [new HtmlWebpackPlugin({chunks, filename: `index.html`, template, title, config})]
  }
}
...
exports.CopyFavicon = (type, source, buildName) => ({
  plugins: [new CopyWebpackPlugin([
    {
      from: path.resolve(__dirname, '../app/assets/images'),
      to: path.resolve(__dirname, `../build/${source}-${type}-${buildName}/images`)
    },
    {
      from: path.resolve(__dirname, '../app/assets/fonts'),
      to: path.resolve(__dirname, `../build/${source}-${type}-${buildName}/fonts`)
    }
  ])]
})
exports.HMR = () => ({
...
exports.loadConstants = (isProd, isStaging, isQA) => {
  // console.log('Staging' + isStaging) console.log('prod' + isProd)
  const config = flatten(Object.assign(baseConfig, isProd
    ? prodConfig
    : isStaging
      ? stagingConfig
